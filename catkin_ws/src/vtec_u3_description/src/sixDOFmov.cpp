 ...
 namespace gazebo 
 {
   ...
   void sixDOFmove::UpdateChild() 
   {
     ...
     parent_->SetAngularVel(ignition::math::Vector3(roll_, pitch_, yaw_));
   ...
   void sixDOFmove::publishOdometry(double step_time) 
   {
     ...
     {
       float last_roll = last_odom_pose_.Rot().Roll();
       float current_roll = pose.Rot().Roll();
     ...
     {
       float last_pitch = last_odom_pose_.Rot().Pitch();
       float current_pitch = pose.Rot().Pitch();
